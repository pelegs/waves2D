...
dimx = int(argv[1])  # width of the simulation domain
dimy = int(argv[1])  # height of the simulation domain
...
def update(u: any, method: int, tick):
    ...
    f = float(argv[3])
    g_rad = 5
    osc = amp * np.sin(np.radians(tick * ts) * f * np.pi * 2)
    pos = np.array(pygame.mouse.get_pos()).astype(int)
    if g_rad < pos[0] < dimx - g_rad and g_rad < pos[1] < dimy - g_rad:
        u[
